...
flashMonitorExeption::flashMonitorExeption(const std::string& message)
	:std::runtime_error(message)
{}

std::auto_ptr<flashMonitor> flashMonitor::instanse_;
const std::wstring flashMonitor::class_name(L"UsbMonWinClass");

flashMonitor::flashMonitor(HINSTANCE hInstanse, bool monitor_hard_drives)
	:started_(false),
	...
	mon_hwnd_(NULL)
{
	...
	if (!RegisterClassExW(&wndClass))
		throw flashMonitorExeption("Cannot register window class");
	...
	if (mon_hwnd_ == NULL)
	{
		...
		throw flashMonitorExeption("Cannot create window");
	}

	SetWindowLongPtrW(mon_hwnd_, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
}

INT_PTR WINAPI flashMonitor::WinProcCallback(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
...
BOOL flashMonitor::devices_changed(WPARAM wParam, LPARAM lParam)
{
	if (started_)
	{
				...
				if (it != existing_notifications_.end())
				{
					...
					if (it != existing_notifications_.end())
					{
						if (safe_remove_on_timeout_ && (InSendMessageEx(NULL) & ISMEX_REPLIED))
						{
							...
							catch (const flashMonitorExeption&)
							{
...
void flashMonitor::detect_changed_devices()
{
...
void flashMonitor::mount_device(wchar_t letter)
{
...
void flashMonitor::unmount_device(wchar_t letter, bool call_unsafe_callback)
{
	for (notifications::iterator handle_it = existing_notifications_.begin(); handle_it != existing_notifications_.end(); ++handle_it)
	{
		if ((*handle_it).second.second == letter)
...
void flashMonitor::unmount_all_devices()
{
...
void flashMonitor::mount_existing_devices()
{
	std::set<wchar_t> devices(get_flash_disks(monitor_hard_drives_));
	for (std::set<wchar_t>::const_iterator it = devices.begin(); it != devices.end(); ++it)
		mount_device(*it);
...
void flashMonitor::on_device_add()
{
	on_device_added_.clear();
}

void flashMonitor::on_device_remove()
{
	on_device_removed_.clear();
}

void flashMonitor::on_device_safe_remove()
{
	on_device_safe_removed_.clear();
}

void flashMonitor::on_device_remove_fail()
{
	on_device_remove_failed_.clear();
}

flashMonitor* flashMonitor::create( HINSTANCE hinst, bool monitor_hard_drives)
{
	if (!instanse_.get())
		instanse_.reset(new flashMonitor(hinst,monitor_hard_drives));

	return instanse_.get();
}

void flashMonitor::remove()
{
	instanse_.reset(0);
}
...
std::set<wchar_t> flashMonitor::get_flash_disks(bool include_usb_hard_drives)
{
...
void flashMonitor::message_pump()
{
	MSG msg;
	int retVal;
...
void flashMonitor::start()
{
	started_ = true;
}

void flashMonitor::stop()
{
	started_ = false;
}


const flashMonitor::device_info flashMonitor::get_device_info(char letter)
{
	...
	if (vol == INVALID_HANDLE_VALUE)
		throw flashMonitorExeption("Cannot open device");
	...
	CloseHandle(vol);
	if (DeviceNumber == -1)
		throw flashMonitorExeption("Cannot get device number");
	...
	if (!QueryDosDeviceW(devname, dos_name, MAX_PATH))
		throw flashMonitorExeption("Cannot get device info");
	...
	default:
		throw flashMonitorExeption("Unknown device");
	}

	HDEVINFO dev_info = SetupDiGetClassDevsW(guid, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);

	if (dev_info == INVALID_HANDLE_VALUE)
		throw flashMonitorExeption("Cannot get device class");
	...
	if (!found)
		throw flashMonitorExeption("Cannot find device");

	DEVINST dev_parent = 0;
	if (CR_SUCCESS != CM_Get_Parent(&dev_parent, spdd.DevInst, 0))
		throw flashMonitorExeption("Cannot get device parent");
...
void flashMonitor::safe_eject(char letter)
{
	...
	if (CR_SUCCESS != CM_Request_Device_EjectW(dev, NULL, NULL, 0, 0))
		throw flashMonitorExeption("Cannot safe-eject device");
}

void flashMonitor::set_safe_remove_on_timeout(bool remove)
{
	safe_remove_on_timeout_ = remove;
}

bool flashMonitor::is_set_safe_remove_on_timeout_on() const
{
	return safe_remove_on_timeout_;
}

bool flashMonitor::is_started() const
{
	return started_;
}

void flashMonitor::enable_usb_hard_drive_monitoring(bool enable)
{
	monitor_hard_drives_ = enable;
}

bool flashMonitor::is_usb_hard_drive_monitoring_enabled() const
{
